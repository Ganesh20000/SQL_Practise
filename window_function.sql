...
SELECT * FROM
(SELECT *,
LAST_VALUE(NAME) OVER W AS 'NAMES',
LAST_VALUE(MARK) OVER W AS 'MARKS'

FROM MARK
WINDOW W AS (PARTITION BY BRANCH ORDER BY MARK  DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING)

) T
WHERE T.MARK=T.MARKS AND T.NAME=T.NAMES;
...
-- FIND THE MOM REVENUE     
USE SUBQUERY;
SELECT *,
LAG(MARK) OVER(PARTITION BY BRANCH ORDER BY MARK),
LEAD(MARK) OVER( PARTITION BY BRANCH  ADDORDER BY MARK) 
FROM MARK;

-- FIND THE MOM GROWTH OF ZOMATO 

SELECT MONTHNAME(DATE), MONTH(DATE),SUM(AMOUNT) FROM ORDERS
GROUP BY MONTHNAME(DATE),MONTH(DATE)
ORDER BY MONTH(DATE) ASC;


SELECT MONTHNAME(DATE), SUM(AMOUNT) FROM ORDERS
